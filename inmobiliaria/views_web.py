...
class ContratoCreateView(AppLoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        pendiente = aplicar_validacion_formato_o_plan(form.instance, self.request.user)
        ...
        if pendiente:
            messages.warning(
                self.request,
                f"Plan de pagos #{self.object.numero} en borrador pendiente de "
                "validación de admin/gerencia (rol Proyectos, lotes y mapa).",
            )
        elif n:
            messages.success(
                self.request,
...
class ContratoUpdateView(AppLoginRequiredMixin, SensitiveEditMixin, UpdateView):
    ...
    def form_valid(self, form):
        ...
        if pendiente:
            messages.warning(
                self.request,
                "Cambios del plan en borrador pendientes de validación de admin/gerencia "
                "(rol Proyectos, lotes y mapa).",
            )
        elif n:
            messages.success(
                self.request,
...
class FormatoAceptacionUpdateView(
    ...
    def form_valid(self, form):
        ...
        if pendiente:
            messages.warning(
                self.request,
                "Cambios del formato pendientes de validación de admin/gerencia "
                "(rol Proyectos, lotes y mapa).",
            )
        else:
